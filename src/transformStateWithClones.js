...
function transformStateWithClones(state, transforms) {
  ...
  for (const trans of transforms) {
    ...
    if (trans.operation === 'clear') {
      for (const key in trueCopy) {
        if (trueCopy.hasOwnProperty(key)) {
          delete trueCopy[key];
        }
      }
      arr.push({ ...trueCopy });
